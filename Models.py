...
class LSTM_Encoder(nn.Module):
    def __init__(self, embedding_dim, lstm_dim, dropout, vocab_size=21):
...
class CNN_Encoder(nn.Module):
    def __init__(self, embedding_dim, encoding_dim,do, vocab_size=21):
        super(CNN_Encoder, self).__init__()
        self.embedding_dim = embedding_dim
        self.encoding_dim = encoding_dim
        self.dim = 128
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
        self.conv1 = nn.Sequential(nn.Conv1d(self.embedding_dim, 128, kernel_size=3, padding=0), nn.ReLU(), nn.MaxPool1d(3))
        self.conv2 = nn.Sequential(nn.Conv1d(128, 256, kernel_size=2,  padding=0), nn.ReLU(), nn.MaxPool1d(2))
        # self.conv3 = nn.Sequential(nn.Conv1d(64, 64, kernel_size=1, padding=0), nn.ReLU(), nn.MaxPool1d(2))
        self.fc2 = nn.Sequential(nn.Linear(256, 256), nn.ReLU(), nn.Dropout(do))
        self.fc3 = nn.Linear(256, self.encoding_dim)
